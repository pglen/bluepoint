...
{
    int ret = 0;  int loop;
    char newpass[PASSLIM + 2];

    // Warning last char not encrypted
    if (blen % 2)
        {
        blen --; ret = 1;
        }
...

{
    ...
    //for(int aa = 0; aa < PASSLIM; aa++)
    //    {
    //    printf("%x ", newpass[aa] & 0xff);
    //    }
    //printf("\n");
    ...
    char    *str = newpass; int slen = PASSLIM;
    ...
    for (int aa = 0; aa < 10; aa++)
        ...
        for(int loop = 0; loop < items; loop++)
            ...
            if(idx < sizeof(vmstack) / sizeof(void*) )
                {
                //printf("vmstack: %d\n", idx);
                vmstack[idx](newpass, PASSLIM, "123412341234", 12);
                }
            //else
...
void    ENCRYPT(char *str, int slen, char *pass, int plen)
{
    ...
    #if 1
    for(int loop2 = 0; loop2 < plen; loop2++)
        {
        int idx = midx_list[abs(pass[loop2]) % maxvec];
        //printf("vm %d idx: %d\n", loop2, idx);
        if(idx < sizeof(vmstack) / sizeof(void*) )
            vmstack[idx](str, slen, pass, plen);
        }
...
void    DECRYPT(char *str, int slen, char *pass, int plen)
{
    ...
    #if 1
    //for(int loop2 = 0; loop2 < plen; loop2++)
    for(int loop2 = plen - 1; loop2 >= 0; loop2--)
        {
        int idx = midx_list[abs(pass[loop2]) % maxvec];
        //printf("vmr %d idx: %d\n", loop2, idx);
